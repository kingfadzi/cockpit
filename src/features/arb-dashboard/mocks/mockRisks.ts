...
export function generateAutoGeneratedRisk(
  appName: string,
  arbDomain: 'security' | 'data' | 'operations' | 'enterprise_architecture' | 'resilience',
...
export function generateManualRisk(
  ...
  const manualRiskTitles = {
    ...
    resilience: [
      ...
      'Inadequate High Availability Configuration'
    ]
  };
...
export function generateRisksForApp(
  ...
  for (let i = 0; i < autoRiskCount; i++) {
    ...
    const domain = Math.random() > 0.7 ?
      getRandomItem<Risk['arbDomain']>(['security', 'data', 'operations', 'enterprise_architecture', 'resilience'])! :
      primaryDomain;
