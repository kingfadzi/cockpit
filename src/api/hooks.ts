...
export const useApps = () =>
    useQuery<AppSummary[]>({ queryKey: ['apps'], queryFn: () => endpoints.listApps() });

export const useApp = (appId: string) =>
    useQuery<AppSummary>({ queryKey: ['apps', appId], queryFn: () => endpoints.getApp(appId), enabled: !!appId });

export const useProfile = (appId: string) =>
    useQuery<any>({ queryKey: ['profile', appId], queryFn: () => endpoints.getProfile(appId), enabled: !!appId });

export const useEvidence = (appId: string) =>
    useQuery<EvidenceItem[]>({ queryKey: ['evidence', appId], queryFn: () => endpoints.getEvidence(appId), enabled: !!appId });

export const useAllEvidence = () =>
    useQuery<EvidenceItem[]>({ queryKey: ['evidence', 'all'], queryFn: () => endpoints.getAllEvidence() });
...
export const useReleases = (appId: string) =>
    useQuery<ReleaseItem[]>({
      queryKey: ['releases', appId],
      queryFn: () => endpoints.getReleases(appId),
      enabled: !!appId,
    });

export const usePortfolioKpis = () =>
    useQuery<PortfolioKpis>({ queryKey: ['kpis', 'portfolio'], queryFn: () => endpoints.getPortfolioKpis() });
